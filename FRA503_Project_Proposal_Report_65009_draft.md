...
| 3.2 | ทดสอบความถูกต้องของการประมาณ Cadence จาก Gyroscope | หา gyro bias และแกนหมุนจริงของขาจานในเฟรมของ IMU จากนั้น project ค่า gyro ทั้งสามแกนไปยังแกน crank ด้วย dot product เพื่อคำนวณ RPM และเปรียบเทียบกับค่า RPM อ้างอิงจากวิดีโอ | รอทำหลัง 3.1 |
...
## 3.2 การทดลองทดสอบความถูกต้องของการประมาณ Cadence จาก Gyroscope

### 3.2.1 วัตถุประสงค์

- ประเมินความถูกต้องของการคำนวณ cadence หรือรอบต่อนาทีของขาจานจาก MPU6050 gyroscope
- หา gyro bias ของแต่ละแกนก่อนคำนวณค่า angular velocity ที่ใช้งานจริง
- หาแกนหมุนของขาจานในเฟรมของ IMU จากข้อมูล gyroscope ทั้งสามแกน
- เปรียบเทียบค่า `RPM_IMU` กับค่า `RPM_ref` ที่คำนวณจากวิดีโออ้างอิง

### 3.2.2 ตัวแปรที่ใช้ในการทดลอง

- `gyro_x`, `gyro_y`, `gyro_z`: ค่า gyroscope ดิบจาก MPU6050
- `bias_x`, `bias_y`, `bias_z`: ค่า bias ขณะ IMU หยุดนิ่ง
- `g_x`, `g_y`, `g_z`: ค่า gyroscope หลังชดเชย bias
- `u_crank`: เวกเตอร์หนึ่งหน่วยตามแกนหมุนจริงของขาจานในเฟรม IMU
- `omega_crank`: อัตราการหมุนของขาจานหลัง project ลงแกน crank
- `RPM_IMU`: ค่า RPM ที่คำนวณจาก IMU
- `RPM_ref`: ค่า RPM อ้างอิงจากวิดีโอ
- `Error(%)`: ค่าความคลาดเคลื่อนของ `RPM_IMU` เทียบกับ `RPM_ref`

### 3.2.3 อุปกรณ์ที่ใช้

- ESP32 NodeMCU-32S / ESP-32S Kit
- MPU6050 ที่ติดตั้งบน crank arm
- จักรยานหรือชุดขาจานที่สามารถหมุนได้อย่างต่อเนื่อง
- คอมพิวเตอร์สำหรับบันทึก Serial log หรือ CSV
- โทรศัพท์หรือกล้องสำหรับบันทึกวิดีโออ้างอิง
- โปรแกรมสำหรับดูเวลาในวิดีโอแบบ frame-by-frame

### 3.2.4 หลักการคำนวณ

ชดเชย gyro bias ของแต่ละแกน:
...
กำหนดเวกเตอร์ gyroscope หลังชดเชย bias:

```text
g = [g_x, g_y, g_z]
```

หาเวกเตอร์ทิศทางการหมุนเฉลี่ยของขาจาน:

```text
v = [mean(g_x), mean(g_y), mean(g_z)]
```
...
project ค่า gyroscope ลงบนแกน crank ด้วย dot product:

```text
omega_crank = g · u_crank
omega_crank = g_x u_x + g_y u_y + g_z u_z
```

คำนวณ RPM จาก IMU:

```text
RPM_IMU = |omega_crank| × 60 / 360
```

คำนวณ RPM อ้างอิงจากวิดีโอ:

```text
RPM_ref = (N_rev / delta_t) × 60
```

คำนวณความคลาดเคลื่อน:

```text
Error(%) = |RPM_IMU_avg - RPM_ref| / RPM_ref × 100
```

### 3.2.5 ขั้นตอนการทดลอง

1. คาลิเบรตค่า Gyro Bias
   - วางจักรยานหรือ crank arm ให้นิ่ง
   - บันทึกค่า `gyro_x`, `gyro_y`, `gyro_z` อย่างน้อย 5 วินาที
   - คำนวณค่าเฉลี่ยเป็น `bias_x`, `bias_y`, `bias_z`

2. หาแกนหมุนของขาจานในเฟรมของ IMU
   - หมุนขาจานด้วยความเร็วคงที่ระดับปานกลาง
   - บันทึกข้อมูล `g_x`, `g_y`, `g_z`
   - คำนวณ `v` และ normalize เป็น `u_crank`

3. บันทึกข้อมูล IMU ระหว่างการหมุนขาจาน
   - บันทึก Serial log หรือ CSV พร้อม timestamp
   - เก็บข้อมูลอย่างน้อย 10 รอบต่อ trial
   - คำนวณ `omega_crank` และ `RPM_IMU` ของแต่ละช่วงเวลา

4. คำนวณค่า RPM อ้างอิงจากวิดีโอ
   - ถ่ายวิดีโอให้เห็นตำแหน่งขาจานครบทุก trial
   - นับจำนวนรอบ `N_rev` จากวิดีโอ
   - วัดเวลา `delta_t` สำหรับ 10 รอบ และคำนวณ `RPM_ref`

5. เปรียบเทียบค่า RPM จาก IMU กับค่าอ้างอิง
   - คำนวณ `RPM_IMU_avg` ของแต่ละ trial
   - คำนวณ `Error(%)`
   - สรุปว่า IMU ให้ค่า cadence อยู่ในเกณฑ์ที่ยอมรับได้หรือไม่

### 3.2.6 รูปแบบการทดลอง

| Trial | รูปแบบการหมุน | จำนวนรอบอ้างอิง |
| ---: | --- | ---: |
| 1 | หมุนช้า | 10 รอบ |
| 2 | หมุนความเร็วปกติ | 10 รอบ |
| 3 | หมุนเร็วขึ้น | 10 รอบ |

### 3.2.7 ตารางบันทึกผลการทดลอง

| Trial | รูปแบบการหมุน | N_rev | delta_t (s) | RPM_ref | RPM_IMU_avg | Error(%) | หมายเหตุ |
| ---: | --- | ---: | ---: | ---: | ---: | ---: | --- |
| 1 | หมุนช้า | 10 | | | | | |
| 2 | หมุนความเร็วปกติ | 10 | | | | | |
| 3 | หมุนเร็วขึ้น | 10 | | | | | |

### 3.2.8 เกณฑ์การประเมินผล

- สามารถคำนวณ `bias_x`, `bias_y`, `bias_z` ได้จากข้อมูลขณะหยุดนิ่ง
- สามารถหา `u_crank` จากข้อมูล gyroscope ทั้งสามแกนได้
- ค่า `RPM_IMU` เปลี่ยนตามระดับความเร็วการหมุนในแต่ละ trial อย่างสมเหตุสมผล
- ค่า `Error(%)` เฉลี่ยไม่เกิน 10% เมื่อเทียบกับ `RPM_ref`
- มี Serial log, CSV หรือวิดีโออ้างอิงประกอบผลการทดลอง
